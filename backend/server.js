...
app.post("/api/register", authLimiter, async (req, res) => {
  try {
    ...
    const verifyUrl = `${baseUrl}/api/verify-email?token=${rawToken}&email=${encodeURIComponent(
      email
    )}`;
...
app.post("/api/verify-otp", (req, res) => {
  try {
    ...
    if (!record)
      return res
        .status(400)
        .json({ error: "No OTP request found. Request a new one." });
    if (Date.now() > record.expires)
...
app.post("/api/forgot-password", authLimiter, async (req, res) => {
  try {
    ...
    if (users[email]) {
      ...
      const baseUrl = process.env.BASE_URL || "http://localhost:3000";
      const resetUrl = `${baseUrl}/login.html?action=reset&token=${rawToken}&email=${encodeURIComponent(
        email
      )}`;
      await transporter.sendMail({
        from: `"COD1 Ecosystem" <${process.env.GMAIL_USER}>`,
...
app.listen(PORT, () =>
  console.log(`✅ COD1 backend running → http://localhost:${PORT}`)
);
